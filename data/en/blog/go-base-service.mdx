...
### First time
When we built a digital product - web/app, firstly we designed a backend system. We must decide to select a backend programming language or some programming language to make this architecture happen. Me too, I know Golang is very strong in building high-performance backend systems with fast compilers, strong parallel processes with go-routine and support blockchain data. Especially, we used Ethereum and Binance Smart Chain node run with the geth node. So, we choose Go. But I think the expertise of the backend team is the most important reason, now JS, Python, Golang, and Rust can build backend systems well. We choose Golang!

To build some services with Golang. We will choose a framework to build this, with some ideas Gin, Go-micro, Echo, Beego, Go-Chi... However, when I go around these frameworks, I need and must have to append more functions in the codebase to build service. So, we use Go-chi (simple router) and build more functions on the codebase. Based on it, my teams can build and deliver specific business services quickly. 

Link Github Here. You if like the repo and thing it helpful, give me a start because it is free 😀
...
## Basic System
I built this go-base-service, which you can use for your goal like User Service, Message Service, and Search Service ... 
...
- Support full-text search using Elastic search's API
